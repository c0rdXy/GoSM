...
func performBlindInjection() {
	...
	if Dump {
		// 输出所有信息
		BooleanBasedBlind2.GetAllInfo(TargetURL)
	} else {
		// 根据命令行参数输出相应的内容
		if Database == "" && Table == "" && Column == "" {
			// 输出所有数据库名
			BooleanBasedBlind2.GetCurrentDatabase(TargetURL)
			BooleanBasedBlind2.GetAllDatabases(TargetURL)
		} else if Database != "" && Table == "" && Column == "" {
			output.PrintCurrentDatabase(Database)
			// 输出某个数据库的所有表名
			BooleanBasedBlind2.GetTablesInDatabase(TargetURL, Database)
		} else if Database != "" && Table != "" && Column == "" {
			output.PrintCurrentDatabase(Database)
			output.PrintTable(Table)
			// 输出某个表的所有字段名
			BooleanBasedBlind2.GetColumnsInTable(TargetURL, Database, Table)
		} else if Database != "" && Table != "" && Column != "" {
			output.PrintCurrentDatabase(Database)
			output.PrintTable(Table)
			output.PrintColumn(Column)
			// 输出某个字段的所有值
			BooleanBasedBlind2.GetColumnsAndValues(TargetURL, Database, Table, Column)
		}
	}
}
...
func performErrorBasedInjection() {
	...
	if Dump {
		// 输出所有信息
		ErrorBased.GetAllInfo(TargetURL)
	} else {
		// 根据命令行参数输出相应的内容
		if Database == "" && Table == "" && Column == "" {
			// 输出所有数据库名
			ErrorBased.GetCurrentDatabase(TargetURL)
			ErrorBased.GetAllDatabases(TargetURL)
		} else if Database != "" && Table == "" && Column == "" {
			// 输出某个数据库的所有表名
			ErrorBased.GetTablesInDatabase(TargetURL, Database)
		} else if Database != "" && Table != "" && Column == "" {
			// 输出某个表的所有字段名
			ErrorBased.GetColumnsAndValuesInTable(TargetURL, Table)
		} else if Database != "" && Table != "" && Column != "" {
			// 输出某个字段的所有值
			ErrorBased.GetColumnsAndValuesInTable(TargetURL, Column)
		}
	}
